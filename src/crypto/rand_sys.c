...
#ifdef DEV_RANDOM_NAME
static int
btc_open(const char *name, int flags) {
  int fd;
...
#ifdef HAVE_EGD
static int
btc_socket(int domain, int type, int protocol) {
  int fd;
...
#ifdef DEV_RANDOM_NAME
static int
btc_devrand(void *dst, size_t size, const char *name) {
  unsigned char *data = (unsigned char *)dst;
  size_t max = INT_MAX;
  struct stat st;
  int fd, nread;
  ...
  if (strcmp(name, "/dev/urandom") == 0) {
    do {
      fd = btc_open("/dev/random", O_RDONLY);
    } while (btc_retry_open(fd));
...
btc_sysrand(void *dst, size_t size) {
  ...
  if (btc_callrand(dst, size))
    goto done;

#ifdef DEV_RANDOM_NAME
  if (btc_devrand(dst, size, DEV_RANDOM_NAME))
    goto done;
#endif

#ifdef HAVE_SYSCTL_UUID
  if (btc_uuidrand(dst, size))
    goto done;
#endif

#ifdef HAVE_EGD
  if (btc_egdrand(dst, size))
    goto done;
#endif
