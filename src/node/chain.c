...
static int64_t
btc_chain_now(btc_chain_t *chain) {
  if (chain->td == NULL)
    return btc_now();

  return btc_timedata_now(chain->td);
}
...
btc_chain_throw(btc_chain_t *chain,
  ...
  btc_chain_log(chain, "Chain error: code=%s reason=%s score=%d malleated=%d",
